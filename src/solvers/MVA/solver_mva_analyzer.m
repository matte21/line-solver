function [Q,U,R,T,C,X,lG,runtime] = solver_mva_analyzer(qn, options)
...
switch options.method
    case {'exact','mva'}
        if all(isfinite(Nchain)) % if closed
            [~,~,Rchain,Tchain,~, Xchain, lG] = solver_mva(STchain, Vchain, Nchain, S, options, qn.schedid, refstatchain);
    ...
    case {'default','amva'}
        [~,Uchain,Rchain,Tchain,~, Xchain] = solver_amva(STchain, Vchain, Nchain, S, SCVchain, options, qn.schedid, qn.schedparam, refstatchain);
