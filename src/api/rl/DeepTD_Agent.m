% TD agent with function approximation for the Q and value function 

classdef DeepTD_Agent < handle
    properties
        v;                      % value function
        Q;                      % Q function
    ...
    methods
        function initialise(obj, lr, eps, epsDecay)    % stateSize: max jobs in a queue; actionSize: number of queues
            obj.lr = lr;
            obj.epsilon = eps;
...
%         end
        ...
        function solve(obj, env)
            ...
            replayBuffer = [];
            obj.Q = obj.createQFunctionNN(env);
            ...
            while j < num_episodes
                ...
                t_prev = t;
                [t, depNode] = env.sample();                                       % how to successive sampling
                dt = t - t_prev;
        ...
        % use NN to approximate the Q-fn;
        % customise the architecture of NN here
        function n = createQFunctionNN(obj, env)
            n = net();
    ...
    methods(Static)
        function policy = createGreedyPolicy(state_Q, epsilon, nA)
             ...
             [~, argmin] = min(state_Q);
             policy(argmin) = policy(argmin) + 1 - epsilon;
        end
