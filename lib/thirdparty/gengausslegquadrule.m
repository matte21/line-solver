function [ zerosave,w,reccalls ] = gengausslegquadrule( nmax,tol,numpoint,reccalls,maxrec)
...
% https://www.mathworks.com/matlabcentral/fileexchange/35886-gauss-laguerre-quadrature
%% check inputs
if nargin==1
...
if nargin <5
    maxrec = 8; %maximum number of recalls to achieve convergence
end
if tol(1) < 5*eps && reccalls ==0
    warning('this position tolerance is of order eps, function may fail')
end
if nargin < 3
    numpoint = 100*(nmax+5); %usually an adequate amount 
end
if nmax ==0
...
if zcount < nmax
   ...
   warning('missed zeros, doubling number of points')
    ...
    x = sqrt(x);
    [ zerosave,w,reccalls ] = gengausslegquadrule( nmax,tol,x,reccalls,maxrec );
    return
...
%     finex = finex( diff(finex)>eps); %if these points are not distinct, remove them
    ...
    clear tmp1 tmp2
    reccalls = reccalls+1;
   [ zerosave,w,reccalls ] = gengausslegquadrule( nmax,tol,finex,reccalls,maxrec ); 
   return
else
