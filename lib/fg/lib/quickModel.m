...
%QUICKMODEL Generate simple closed queueing network based on given parameters
    ...
    if ~exist('jobs','var')
        % jobs parameter does not exist, so default it to one
    ...
    for c=1:numClasses
        if ~open
            ...
            if ~exist('routing','var')
                ...
                P{c} = simpleRoutes([setdiff(1:size(simpleRoutes,1), [1]), [1]], :);
            else
                P{c} = routing(c, :);
            end
        else
        ...
        for i=1:numStations
            node{i}.setService(jobclass{c}, Exp.fitMean(classes(c,i)));
        end
    end
