...
node{3}.setService(jobclass{1}, Exp.fitMean(3.5)); % mean = 1.5
node{3}.setService(jobclass{2}, Exp.fitMean(4.5)); % mean = 1.5
node{3}.setNumberOfServers(c);
...
node{3} = Queue(ldmodel, 'Queue2', SchedStrategy.PS);
...
node{3}.setService(jobclass{1}, Exp.fitMean(3.5));
node{3}.setService(jobclass{2}, Exp.fitMean(4.5));
node{3}.setLoadDependence(min(1:(N+N/2),c)); % multi-server with c servers
